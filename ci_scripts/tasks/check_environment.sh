...
check_swiftlint_environment() {
  local project_file="Liet.xcodeproj/project.pbxproj"
  ...
  if ! grep -q --fixed-strings "https://github.com/SimplyDanny/SwiftLintPlugins" "$project_file"; then
    record_failure "Liet.xcodeproj is missing the SwiftLintPlugins package dependency."
    record_next_step "Add https://github.com/SimplyDanny/SwiftLintPlugins to Liet.xcodeproj."
  fi
}
